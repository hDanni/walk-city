<template>
  <div id="loc-comp-background">
    <router-link
      :to="{
        name: 'locationDetail',
        params: { locationId: location.businessId },
      }"
    >
      <div id="loc-container" :class="{'checked-in': filterCheckIn(location.businessId)}">
        <h1 class="loc-name">{{ location.name }}</h1>
        <!-- <h2 class="loc-hours">{{location.days}} {{location.hours}}</h2> -->
        <h2 class="loc-hours">{{ location.address }}</h2>
        <img class="loc-img" v-bind:src="location.imageUrl" alt="" />
        <h3 class="loc-cat">{{ location.category }}</h3>
